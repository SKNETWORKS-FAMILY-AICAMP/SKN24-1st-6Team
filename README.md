...
## 1. 👥 팀 소개
### ✨ 구갱 (Gu-Gang) ✨
- 팀원들이 90년대생이라는 데이터로 이루어진 👉 9 Gang 😎🔥
> 💪😆 90’s + Gang = 구갱 ✨🔫
...
**CARPORT**는 **2025년** 최신 데이터를 바탕으로, **자동차 구매를 고민하거나 시장 동향이 궁금한 분**들을 위해 **전국 자동차 등록 현황**과 **인기 모델 정보, FAQ**를 한눈에 보여주는 플랫폼입니다. 
...
**김정현**
>길지 않은 시간임에도 불구하고 Selenium 툴을 이용하여 신규등록 수 데이터를 수집하는 구조를 만들고 이를 통해 적재된 데이터들의 테이블을 추출, Streamlit 웹 페이지를 구현하는 일련의 과정을 완성한 것이 만족스럽습니다. 다만, 데이터 수집과 관련해 사전에 충분히 논의되지 못하여 코드 구현 시 이슈가 되기도 했고, Streamlit 웹 페이지의 UI를 더 다듬지 못해 아쉽습니다. 업무 분담 및 사전 논의 사항에 대해 진지하게 고민하고 정해서 진행해야겠고, Streamlit을 더 자세히 살펴보고 싶습니다.

**정재훈**
>이론과 간단한 실습으로만 배운 Python(데이터크롤링, Selenium), MYSQL, streamlit을 모두 연결하여 사용할 수 있는 프로젝트를 수행한 것 같아서 큰 의미가 크다고 생각한다.
(1) 크롤링하는 부분에서 1페이지에서 여러 규칙과 조건을 적용하여 코드를 구성하는 것, (2) 추출한 텍스트 데이터를 DB에 적재하기 위해서 PK, FK를 고려하여 직접 연결, (3) Streamlit을 통한 홈페이지 구성 경험을 통해 실무에서도 활용 가능한 정도인 것 같았다고 느꼈고, 개인적으로 수업을 듣기전과 다르게 많이 성장했다고 느꼈다.
다만 아쉬운 점은, 시간이 조금 더 있었으면 더욱 더 많은 사이트의 데이터(정량, 정성)를 크롤링해서 DB에 저장하여 대량의 데이터를 구축하면 좋을 것 같다고 생각했다. 시간이 날때마다 크롤링을 주기적으로 해보려고 한다.
...
**박정은**
>팀원 간의 끊임없는 소통과 전원 투입 덕분에 짧은 기간 내에 프로젝트를 무사히 완수할 수 있었다고 생각합니다. 특히 서로의 업무 현황을 계속 공유하며 보조를 맞춘 점이 큰 힘이 되었습니다. 다만 초기에 데이터 수집 및 MySQL 표준 가이드를 (example: 저장 자동화, 등등) 먼저 정립했다면 더욱 효율적이었을 것이라는 아쉬움이 남습니다. 이번 경험을 발판 삼아 다음에는 더 다양한 연도의 데이터를 확보하고, 프로젝트 규모를 키우며(scaling) DB 정규화 과정을 제대로 작업해 보고 싶습니다.
